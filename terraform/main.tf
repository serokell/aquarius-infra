...
resource "aws_route53_record" "bunda_aquarius_serokell_team_ipv4" {
  zone_id = aws_route53_zone.aquarius_serokell_team.zone_id
  name    = "bunda.aquarius.serokell.team"
  type    = "A"
  ttl     = "60"
  records = [hcloud_server.bunda.ipv4_address]
}

resource "aws_route53_record" "bunda_aquarius_serokell_team_ipv6" {
  zone_id = aws_route53_zone.aquarius_serokell_team.zone_id
  name    = "bunda.aquarius.serokell.team"
  type    = "AAAA"
  ttl     = "60"
  records = [hcloud_server.bunda.ipv6_address]
}
